    ...
    for i in range(0, gray_img.shape[0], interval):
        ...
        for j in range(1, len(row_array)):
            if int(row_array[j]-row_array[j-1])<T:
                ...
                cv2.circle(img1, (j-1, i), 1, (0,0,255), 1)
                cv2.circle(img1, (j, i), 1, (0, 0, 255), 1)
                cv2.circle(img1, (j+1, i), 1, (0, 0, 255), 1)
    ...
    for i in range(0, gray_img.shape[1], interval):
        ...
        for j in range(1, len(col_array)):
            if int(col_array[j]-col_array[j-1])<T:
                ...
                cv2.circle(img1, (i, j-1), 1, (0, 0, 255), 1)
                cv2.circle(img1, (i, j), 1, (0, 0, 255), 1)
                cv2.circle(img1, (i, j+1), 1, (0, 0, 255), 1)
    cv2.imwrite("./visit_map.tif", img1)
    return visit_map
...
def get_accessible_coords_and_class1(pixel_coords, visitmap, spectral_image, train_patch, Init_label, patchsize, angle):
    ...
    return np.array(Augment_coordinates), np.array(Augment_labels)
def draw_circles(image, pixel_coords, class_labels, radius=10, thickness=10):
    """
...
if __name__ == '__main__':
    ...
    date1 = '201812'
    Dates2 = ['201901', '201902', '201903', '201904', '201905', '201906', '201907', '201908', '201909', '201910','201911', '201912',
    ...
    for num_sampling_sample in num_sampling_samples:
        for a in a_s:
            for date2 in Dates2:
                ...
                #建立第一时相中间结果文件夹#
                Date1_Intermediate_result = os.path.join(read_datapath, 'Intermediate results', Classification_id,'{0}'.format(a), '{0}'.format(num_sampling_sample), '{0}'.format(date1))
                ...
                cv2.imwrite(os.path.join(Date1_Intermediate_result,"{0}_augmentation_draw1.tif".format(date1)), After_draw_circles)
                write_txt(After_coor, After_label, '{0}_augmentation1'.format(date1), Date1_Intermediate_result)
                ###########################聚类采样#############################
                Second_coor = selected_coordinates
                Second_label = selected_labels
                for i in range(1,num_class+1):
                    coord_i = After_coor[After_label==i]
                    if int(num_sampling_sample*ratio1[i-1])<50:
                ...
                # 建立第二时相中间结果文件夹#
                Date2_Intermediate_result = os.path.join(read_datapath, 'Intermediate results', Classification_id,'{0}'.format(a), '{0}'.format(num_sampling_sample),'{0}'.format(date2))
