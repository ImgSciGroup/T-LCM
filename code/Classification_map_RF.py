...
path = '../Data/WuHan'
for date in Time:
    img_mat = tifffile.imread(os.path.join(path,"{}.tif".format(date)))
    patch = 7
    #initial
    Init_coor, Init_label = Get_intrinal_sample(os.path.join(path,"Intermediate results/WHU_{0}_initial.txt".format(date)))
    ...
    #sampling
    Init_coor, Init_label = Get_intrinal_sample(os.path.join(path,"Intermediate results/WHU_{0}_sampling.txt".format(date)))
    selected_coordinates = np.array(Init_coor)
    selected_labels = np.array(Init_label)
